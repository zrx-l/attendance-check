...
# ================== 主处理函数（保持不变，只修改了调用 generate_cell_text 的部分） ==================
            ...
            if not skip:
                ...
                if (emp_id, date) in leave_assignment:
                    ...
                    if (emp_id, date) in checkins:
                        ...
                        if times:
                            text += " " + " ".join(times)
                        text += format_hours(cinfo)   # 休假也追加工时
                    if has_remote:
                        text += remote_suffix
                    cell.value = text
                    modified_count += 1
                ...
                text = generate_cell_text(emp_id, date, {}, checkins)  # 此函数已包含 format_hours
                if text is not None:
                    if has_remote:
...
# ================== run_attendance_check 使 leave 可选 ==================
